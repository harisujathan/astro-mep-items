...
public class dasa2{
  ...
  public static PrintStream pw=System.out;
  ...
  public static void main(String args[]) throws Exception{
   ...
   int deg=Integer.parseInt(args[0]);
    ...
    if(args.length>6)x1=Integer.parseInt(args[6]);
    if(args.length>7)x2=Integer.parseInt(args[7]);
    if(args.length>8)trunc=Integer.parseInt(args[8]);
    for(int i=0;i<pli.length;i++) pli[i]*=mul;
    int tot=deg*3600+min*60+sec;
    tot=tot%((x1*60+x2)*9*60);
    //tot=tot%(120*3600);
    int start=0;
    if(tot>=(0*(x1*3600+x2*60)) && tot<(x1*3600+x2*60))
      start=0;
    else if(tot>=(1*(x1*3600+x2*60)) && tot<(2*(x1*3600+x2*60)))
      start=1;
    else if(tot>=(2*(x1*3600+x2*60)) && tot<(3*(x1*3600+x2*60)))
      start=2;
    else if(tot>=(3*(x1*3600+x2*60)) && tot<(4*(x1*3600+x2*60)))
      start=3;
    else if(tot>=(4*(x1*3600+x2*60)) && tot<(5*(x1*3600+x2*60)))
      start=4;
    else if(tot>=(5*(x1*3600+x2*60)) && tot<(6*(x1*3600+x2*60)))
      start=5;
    else if(tot>=(6*(x1*3600+20*60)) && tot<(7*(x1*3600+20*60)))
      start=6;
    else if(tot>=(7*(x1*3600+20*60)) && tot<(8*(x1*3600+20*60)))
      start=7;
    else if(tot>=(8*(x1*3600+20*60)) && tot<(9*(x1*3600+20*60)))
      start=8;

    int ndeg=tot %(x1*3600+20*60);
//    int rem=(x1*3600+20*60)-ndeg;
    float perc=(float)ndeg/(x1*3600+x2*60);
    float st=-1*perc*pli[start];
    pw.println("percentage="+perc);
   // Thread.sleep(3000);

    //print(plans[start],depth,0,(x1*60+x2)*9/60*mul,st,day,"");
   try{
...
/*        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	...
	c2.add(Calendar.DAY_OF_MONTH, ad2);  
	String newDate = sdf.format(c.getTime());    
	String newDate2 = sdf.format(c2.getTime());    
*/
      ...
      if(inc>0){
        ...
        //System.out.println(plans[beg+j]+"  "+newDate+":"+newDate2+"  * ("+round(st,3)+"-"+round(inc,3) +") ");
